...
class BlogGenerator(Generator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.entries = []  # only entries in default language
        self.translations = []
        self.dates = {}
        ...
        super(BlogGenerator, self).__init__(*args, **kwargs)
        self.drafts = []
        signals.entry_generator_init.send(self)

    def generate_feeds(self, writer):
        """Generate the feeds from the current context, and output files."""

        if self.settings.get('FEED_ATOM'):
            writer.write_feed(self.entries, self.context,
        ...
        if self.settings.get('FEED_ALL_ATOM') or \
            ...
            if self.settings.get('FEED_ALL_ATOM'):
                writer.write_feed(all_entries, self.context,
    ...
    def generate_pages(self, writer):
        ...
        # in writer, entries pass first
        self.generate_entries(write)
    ...
    def generate_context(self):
        ...
        self._update_context(('entries', 'dates', 'tags', 'categories',
                              'tag_cloud', 'authors', 'related_posts'))

        signals.entry_generator_finalized.send(self)
